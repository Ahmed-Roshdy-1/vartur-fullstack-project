<template>
  <form
    action="submit"
    @submit.prevent="updateCategorieFun"
...
const SelectValue = ref(props.data.parent_id);
const emit = defineEmits(["update"]);
const updateCategorieFun = async () => {
  const { data, error } = await useFetch("/api/updateCategorie", {
    method: "POST",
    body: {
      id: props.data.id,
      name: name.value,
